...
def test_pathloss(distance):
        result1 = loss.Path_loss(distance)
        result2 = loss.tf_Path_loss(distance)
        print("result1",result1)
        print("result2",result2.numpy())
        print(result1==result2.numpy())
def Test_svd():
...
steering_vector,path_loss,beta=Test_steering_vector_generation(selected_angle,selected_distance)
print("steering_vector",steering_vector)
...
input_single = np.expand_dims(input_single, axis=3)
CSI = tf.complex(input_single[:, :, 6*antenna_size:7*antenna_size, 0], input_single[:, :, 7*antenna_size:8 * antenna_size, 0])
zf_precoding_matrix = loss.zero_forcing(CSI)
zf_precoding_matrix = tf.complex(input_single[:, :, 4*antenna_size:5*antenna_size, 0], input_single[:, :, 5*antenna_size:6 * antenna_size, 0])
print("zf_precoding_matrix",zf_precoding_matrix)
print("csi",CSI)
steering_vector = tf.complex(input_single[:, :, 0*antenna_size:1*antenna_size, 0], input_single[:, :, 1*antenna_size:2 * antenna_size, 0])
#precoding_matrix = abs(loss.simple_precoder(angle.T,distance.T))
#print("precoding_matrix1",precoding_matrix)
#partial = loss.tf_Echo_partial(beta,zf_precoding_matrix,angle)
